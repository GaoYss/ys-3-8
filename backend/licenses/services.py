...
def dashboard_stats():
    ...
    borrow_status_counts = dict(BorrowRecord.objects.values_list("status").annotate(total=Count("id")))
    ...
    return {
        ...
        "pending_approvals": borrow_status_counts.get(BorrowRecord.Status.PENDING, 0),
        "borrowed_records": borrow_status_counts.get(BorrowRecord.Status.BORROWED, 0),
        "overdue_returns": borrow_status_counts.get(BorrowRecord.Status.OVERDUE, 0),
        "by_type": type_counts,
